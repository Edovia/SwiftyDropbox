...
public func setAssertFunc(assertFunc: @escaping (Bool, String) -> Void) {
...
public func nullableValidator<T>(internalValidator: @escaping (T) -> Void) -> (T?) -> Void {
    return { (value: T?) -> Void in
        if let value = value {
...
public func binaryValidator(minLength: Int?, maxLength: Int?) -> (NSData) -> Void {
